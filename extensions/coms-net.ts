...
export default function (pi: ExtensionAPI) {
	// ━━ CLI flags ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
	// Shared identity flags (`cname`, `purpose`, `project`, `color`, `explicit`)
	// are registered by coms.ts in this package. coms-net only registers its
	// own network-specific flags to avoid duplicate-flag conflicts when both
	// extensions are auto-loaded from package.json.
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		if (!serverUrl) {
			// Package-friendly default: stay dormant when no hub is configured.
			// This lets users install pi-to-pi globally without getting startup
			// error noise in sessions where they only use local coms.
			audit("boot_skipped", { reason: "no_server_url", project });
			try { ctx.ui.setStatus("coms-net", ""); } catch { /* ignore */ }
			return;
		}
	...
	function renderPool(width: number, theme: Theme): string[] {
		...
		if (safeWidth < 16) {
			...
			if (identity && remaining >= 1) {
				const rightTag =
					...
					theme.fg("accent", selfQueueLabel) +
					theme.fg("dim", " ━");
		...
		for (const r of rows) {
			...
			if (r.stale) {
				const staleCore = `✗ ${r.name.padEnd(nameWidth)} ${abbreviateModel(r.model || "unknown").padEnd(modelWidth)} [${"-".repeat(barWidth)}] ${pctLabel.padStart(4)}`;
