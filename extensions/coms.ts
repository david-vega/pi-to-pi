...
export default function (pi: ExtensionAPI) {
	...
	//
	// This extension may be stacked with coms-net, which declares overlapping
	// identity flags. We treat duplicate-flag errors as benign.
	const registerFlagSafe = (name: string, def: { description: string; type: "string" | "boolean"; default: any }) => {
		try {
			pi.registerFlag(name, def);
		} catch (err: any) {
			const msg = String(err?.message ?? err ?? "").toLowerCase();
			if (!msg.includes("conflict")) throw err;
		}
	};
	registerFlagSafe("cname", {
